...
def home():
    if "user_id" in session:
        if (request.args.get("user_id") is None) or str(request.args.get("user_id"))==session["user_id"]:#自分
            ...
            #print(data)
            return render_template("home.html",user_id=session["user_id"],user_name=html.escape(session["hn"]+"@"+session["user_id"]),greet=html.escape(greet),url=html.escape(url),birth=html.escape(birth),loc=html.escape(loc),edit=edit)
        else:#他人
            conn=sql.connect(dbname,check_same_thread=False)
            cur=conn.cursor()
            ...
            return render_template("home.html",user_id=html.escape(request.args.get("user_id")),user_name=html.escape(name+"@"+request.args.get("user_id")),greet=html.escape(greet),url=html.escape(url),birth=html.escape(birth),loc=html.escape(loc),edit=edit)
    else:
        return redirect("login")
...
def follow():
    if "user_id" in session:
        ...
        for i in range(len(data)):
            res=res+"\t<tr><td>ハンドルネーム</td><td><a href=\"home?user_id="+html.escape(data[i][2])+"\">"+html.escape(data[i][0])+"</td></tr>\n"
            res=res+"\t<tr><td colspan=\"2\"><pre>"+html.escape(data[i][1])+"</pre></td></tr>\n"
...
def find():
    if "user_id" in session:
        ...
        res=""
        for i in range(len(data)):
            res=res+"\t<tr><td>ハンドルネーム</td><td><a href=\"home?user_id="+html.escape(data[i][1])+"\">"+html.escape(data[i][2])+"</td></tr>\n"
